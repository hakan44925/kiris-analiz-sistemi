...
def analiz_motoru():
    try:
        ...
        for i in range(len(m_pos)): A[1, i] = m_pos[i]
        if 3 in m_type: 
            ank_idx = np.where(m_type == 3)[0][0]
            A[1, len(m_pos)] = 1
        
        m_load = np.sum(py * pk) + np.sum(ms_val)
        for i in range(len(ws)): m_load += (ws[i] * (we[i] - wb[i])) * ((wb[i] + we[i])/2)
        B[1] = m_load
        ...
        for i, maf_x in enumerate(mafsallar):
            ...
            if 3 in m_type and m_pos[ank_idx] < maf_x: A[row, len(m_pos)] = 1
            m_load_maf = np.sum(py[pk < maf_x] * (maf_x - pk[pk < maf_x])) + np.sum(ms_val[mk_pos < maf_x])
        ...
        for i, xi in enumerate(x):
            ...
            if 3 in m_type and xi >= m_pos[ank_idx]: m_val -= reaksiyonlar[len(m_pos)]
        ...
        # 1. ŞEMA (Yayılı yük değerleri ve metreler eklendi)
        ...
        if mafsallar.size > 0:
            axes[0].scatter(mafsallar, [0]*len(mafsallar), color='white', edgecolor='black', s=120, zorder=5, label="Mafsal")

        for k in range(len(ws)):
            rect = plt.Rectangle((wb[k], 0), we[k]-wb[k], 0.6, color='orange', alpha=0.3)
            axes[0].add_patch(rect)
            # Yayılı yük metni ve metre aralığı
            axes[0].text((wb[k]+we[k])/2, 0.7, f"{ws[k]} kN/m", ha='center', fontweight='bold', color='darkorange')
            axes[0].text((wb[k]+we[k])/2, -0.5, f"{wb[k]}m - {we[k]}m", ha='center', fontsize=9, color='orange')

        axes[0].set_ylim(-1, 2)
        axes[0].set_xlim(-0.5, L+0.5)
        axes[0].axis('off')

        # 2. DİYAGRAMLAR (Değerler ve Max noktalar eklendi)
        titles = ["N (Normal Kuvvet) - kN", "V (Kesme Kuvveti) - kN", "M (Eğilme Momenti) - kNm"]
        units = ["kN", "kN", "kNm"]
        data_list = [N, V, M]
        colors = ['green', 'blue', 'red']
        
        # Kritik x noktaları (etiketler için)
        ...
        for i, (ax, t, c, d, u) in enumerate(zip(axes[1:], titles, colors, data_list, units)):
            ...
            # Değerleri Üstüne Yazma (Sadece anlamlı değişim noktalarında)
            for kx in kritik_x:
                idx = np.argmin(np.abs(x - kx))
                val = d[idx]
                if abs(val) > 0.01:
                    ax.text(kx, val, f"{val:.1f} {u}", fontsize=9, fontweight='bold', ha='center', va='bottom' if val>0 else 'top')

            # Maksimum Değer Vurgusu
            max_idx = np.argmax(np.abs(d))
            ax.annotate(f"MAX: {d[max_idx]:.1f} {u}", xy=(x[max_idx], d[max_idx]), xytext=(x[max_idx], d[max_idx]*1.3),
                         arrowprops=dict(arrowstyle='->', color='black'), fontweight='black', color='darkred')

        st.pyplot(fig)
        st.success("Hesaplama ve Görselleştirme Tamamlandı!")

    except Exception as e:
        st.warning("Giriş formatı veya statik sistem hatası. Lütfen değerleri kontrol edin.")
